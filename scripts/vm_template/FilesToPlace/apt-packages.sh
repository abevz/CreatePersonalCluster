...
retry_cmd 3 curl -fsSL "https://pkgs.k8s.io/core:/stable:/v${KUBERNETES_SHORT_VERSION}/deb/Release.key" | gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg
...
retry_cmd 3 curl https://baltocdn.com/helm/signing.asc | gpg --dearmor | tee /usr/share/keyrings/helm.gpg > /dev/null
echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/helm.gpg] https://baltocdn.com/helm/stable/debian/ all main" | tee /etc/apt/sources.list.d/helm-stable-debian.list

# update apt cache with timeout and retry
echo "Updating apt cache..."
retry_cmd 3 timeout 300 apt-get update
echo "Upgrading packages..."
retry_cmd 3 timeout 600 apt upgrade -y

# Install packages from apt with timeout
echo "Installing essential packages..."
retry_cmd 3 timeout 1200 apt install -y \
bash \
curl \
grep \
git \
...
retry_cmd 3 timeout 600 apt install -y \
