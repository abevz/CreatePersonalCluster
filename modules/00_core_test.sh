...
function display_current_context() {
  ...
  if [[ -n "$aws_creds" ]]; then
    (cd "$REPO_PATH/terraform" && eval "$aws_creds tofu workspace list")
  else
    echo "No AWS credentials available - cannot list workspaces"
  fi
}
...
# identify_cache_files() - Finds all cache files that need to be cleared
function identify_cache_files() {
  local cache_files=()
  
  # Add known cache files
  cache_files+=("/tmp/cpc_secrets_cache")
  cache_files+=("/tmp/cpc_env_cache.sh")
  cache_files+=("/tmp/cpc_status_cache_*")
  cache_files+=("/tmp/cpc_ssh_cache_*")
  cache_files+=("/tmp/cpc_tofu_output_cache_*")
  cache_files+=("/tmp/cpc_workspace_cache")
  
  # Return the list
  printf '%s\n' "${cache_files[@]}"
}

# remove_cache_files() - Deletes the identified cache files
...
# log_cache_clearance() - Logs the successful clearing of cache files
function log_cache_clearance() {
  log_success "Cache cleared successfully"
}

# handle_clear_errors() - Handles errors during cache clearing
function handle_clear_errors() {
  log_error "Failed to clear cache files"
}
...
function handle_output_errors() {
  log_error "Failed to parse output"
}
...
function generate_inventory_content() {
  ...
  if command -v jq &>/dev/null; then
    ...
    echo "$json_data" | jq -r 'to_entries[] | "\(.key) ansible_host=\(.value.IP) hostname=\(.value.hostname)"'
  else
...
function get_aws_credentials() {
  ...
  if [[ -n "${AWS_ACCESS_KEY_ID:-}" && -n "${AWS_SECRET_ACCESS_KEY:-}" ]]; then
    creds="export AWS_ACCESS_KEY_ID='$AWS_ACCESS_KEY_ID' && export AWS_SECRET_ACCESS_KEY='$AWS_SECRET_ACCESS_KEY'"
