...
ssh_clear_hosts() {
    ...
    # Get hostnames from cluster-info
    log_info "Fetching hostname information from cluster..."
    local cluster_info
    ...
    if [ -n "$cluster_info" ] && [ "$cluster_info" != "null" ]; then
        ...
        log_warning "  Could not get hostnames from cluster info, using default naming pattern"
        ...
        log_info "  Added default hostnames: ${vm_hostnames_to_clear[*]}"
    fi
...
ssh_get_vm_ips_from_context() {
    ...
    if ! tofu workspace select "$context" &>/dev/null; then
        log_warning "Could not select Tofu workspace '$context'"
        popd > /dev/null
        return 1
    fi
